...
const dependencyUtils = {
   ...
   */
  mergeDependenciesIntoMap(json = {}) {
    return _extends({}, json.dependencies || {}, json.devDependencies || {}, json.optionalDependencies || {});
  },
   ...
   */
  mergeDependenciesIntoList(json = {}) {
   ...
   */
  async updateDependency(packagePath, dependencyKey, version) {
    const update = (obj, dependencyType, dependencyKey, version) => {
   ...
   */
  async hasRootVersionMisMatch(key, packagePath) {
    const rootPath = await path.resolveMonoRepoRootPath();
    const [rootPackageJson, packageJson] = await Promise.all([this.readPackageJson(rootPath), this.readPackageJson(packagePath)]);
   ...
   */
  ensureDependencyScopeExists(key, directoryPath) {
   ...
   */
  async createSymlinkForDependency(key, rootDir, packageDir, relative) {
    ...
    const distDir = this.isScopedDependency(key) ? join(packageDir, 'node_modules', scope) : join(packageDir, 'node_modules');
