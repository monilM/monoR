...
module.exports = async function createFlowTypeSymlinks({ flowConfigPath, relative }, cwd = process.cwd()) {
  ...
  const [rootPath, packagePaths] = await Promise.all([path.resolveMonoRepoRootPath(), path.resolveMonoRepoPackagePaths()]);
  ...
  await Promise.all(packagePaths.map(async packagePath => {
    ...
    await Promise.all(dependencyKeys.filter(key => ignoredPackageKeys.includes(key) === false).map(key => {
